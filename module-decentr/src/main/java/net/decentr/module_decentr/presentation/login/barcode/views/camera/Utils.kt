...
object Utils {
    ...
    fun convertToBitmap(data: ByteBuffer, width: Int, height: Int, rotationDegrees: Int): Bitmap? {
        val IMAGE_FORMAT_NV21 = 17
        data.rewind()
        val imageInBuffer = ByteArray(data.limit())
        data.get(imageInBuffer, 0, imageInBuffer.size)
        try {
            val image = YuvImage(
                imageInBuffer, IMAGE_FORMAT_NV21, width, height, null
            )
    ...
    private fun getExifOrientationTag(resolver: ContentResolver, imageUri: Uri): Int {
        ...
        try {
            resolver.openInputStream(imageUri)?.use { inputStream -> if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {

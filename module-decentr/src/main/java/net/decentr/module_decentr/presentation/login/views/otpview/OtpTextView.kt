...
class OtpTextView : FrameLayout {
    ...
    private fun generateViews(styles: TypedArray, attrs: AttributeSet?) {
        ...
        if (length > 0) {
            ...
            val width =
                styles.getDimension(R.styleable.OtpTextView_width, DEFAULT_WIDTH.toPx().toFloat())
                    .toInt()
            val height =
                styles.getDimension(R.styleable.OtpTextView_height, DEFAULT_HEIGHT.toPx().toFloat())
                    .toInt()
            val weight = styles.getInteger(R.styleable.OtpTextView_weight, DEFAULT_SPACE)
            val space = styles.getDimension(
                R.styleable.OtpTextView_box_margin,
                DEFAULT_SPACE.toPx().toFloat()
            ).toInt()

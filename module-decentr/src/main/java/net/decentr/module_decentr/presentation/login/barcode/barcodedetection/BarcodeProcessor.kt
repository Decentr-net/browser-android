...
package net.decentr.module_decentr.presentation.login.barcode.barcodedetection
...
import net.decentr.module_decentr.presentation.login.barcode.views.camera.CameraReticleAnimator
import net.decentr.module_decentr.presentation.login.barcode.views.camera.FrameProcessorBase
...
import net.decentr.module_decentr.presentation.login.barcode.views.camera.InputInfo
import net.decentr.module_decentr.presentation.login.barcode.views.camera.PreferenceUtils
import java.io.IOException

/** A processor to run the barcode detector.  */
class BarcodeProcessor(graphicOverlay: GraphicOverlay, private val workflowModel: QRLoginViewModel) :
    ...
    override fun onSuccess(
        ...
        if (barcodeInCenter == null) {
            ...
            workflowModel.setWorkflowState(QRLoginViewModel.WorkflowState.DETECTING)
        } else {
            ...
            if (sizeProgress < 1) {
                ...
                workflowModel.setWorkflowState(QRLoginViewModel.WorkflowState.CONFIRMING)
            } else {
                ...
                if (PreferenceUtils.shouldDelayLoadingBarcodeResult(graphicOverlay.context)) {
                    ...
                    workflowModel.setWorkflowState(QRLoginViewModel.WorkflowState.SEARCHING)
                } else {
                    workflowModel.setWorkflowState(QRLoginViewModel.WorkflowState.DETECTED)
        ...
        return ValueAnimator.ofFloat(0f, endProgress).apply {
            ...
            addUpdateListener {
                if ((animatedValue as Float).compareTo(endProgress) >= 0) {
                    graphicOverlay.clear()
                    workflowModel.setWorkflowState(QRLoginViewModel.WorkflowState.SEARCHED)
